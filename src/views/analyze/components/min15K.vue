...
const fetchCodes = analyzeMetaStores.requestMin15KCodes;
const { min15KCodes } = storeToRefs(analyzeMetaStores);

const analyzeKStores = useAnalyzeKline();
const fetchMethod = analyzeKStores.requestMin15K;
const { min15KData, min15KLoading } = storeToRefs(analyzeKStores);
const kLineOptions = ref({})

watch(() => min15KData, kline => {
    let xAxisTime = [];
...
<template>
    ...
    <v-chart style="height: 400px;" :loading="min15KLoading" :autoresize="true" :option="kLineOptions"></v-chart>
</template>
