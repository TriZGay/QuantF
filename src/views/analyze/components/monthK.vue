...
const fetchCodes = analyzeMetaStores.requestMonthKCodes;
const { monthKCodes } = storeToRefs(analyzeMetaStores);

const analyzeKStores = useAnalyzeKline();
const fetchMethod = analyzeKStores.requestMonthK;
const { monthKData, monthKLoading } = storeToRefs(analyzeKStores);
const kLineOptions = ref({})

watch(() => monthKData, kline => {
    let xAxisTime = [];
...
<template>
    ...
    <v-chart style="height: 400px;" :loading="monthKLoading" :autoresize="true" :option="kLineOptions"></v-chart>
</template>
