...
const fetchCodes = analyzeMetaStores.requestMin60KCodes;
const { min60KCodes } = storeToRefs(analyzeMetaStores);

const analyzeKStores = useAnalyzeKline();
const fetchMethod = analyzeKStores.requestMin60K;
const { min60KData, min60KLoading } = storeToRefs(analyzeKStores);
...
<template>
    ...
    <v-chart style="height: 400px;" :loading="min60KLoading" :autoresize="true" :option="kLineOptions"></v-chart>
</template>
