...
const fetchCodes = analyzeMetaStores.requestQuarterKCodes;
const { quarterKCodes } = storeToRefs(analyzeMetaStores);

const analyzeKStores = useAnalyzeKline();
const fetchMethod = analyzeKStores.requestQuarterK;
const { quarterKData, quarterKLoading } = storeToRefs(analyzeKStores);
const kLineOptions = ref({})

watch(() => quarterKData, kline => {
    let xAxisTime = [];
...
<template>
    ...
    <v-chart style="height: 400px;" :loading="quarterKLoading" :autoresize="true" :option="kLineOptions"></v-chart>
</template>
