...
const fetchCodes = analyzeMetaStores.requestWeekKCodes;
const { weekKCodes } = storeToRefs(analyzeMetaStores);

const analyzeKStores = useAnalyzeKline();
const fetchMethod = analyzeKStores.requestWeekK;
const { weekKData, weekKLoading } = storeToRefs(analyzeKStores);
const kLineOptions = ref({})

watch(() => weekKData, kline => {
    let xAxisTime = [];
...
<template>
    ...
    <v-chart style="height: 400px;" :loading="weekKLoading" :autoresize="true" :option="kLineOptions"></v-chart>
</template>
